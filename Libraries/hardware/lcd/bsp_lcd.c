...
uint32_t CurrentFrameBuffer = (uint32_t)s_psBufferLcd[0];
...
void bsp_draw_pixel(uint16_t Xpos, uint16_t Ypos,uint16_t color)
{   
	if ( ( Xpos < LCD_PIXEL_WIDTH ) && ( Ypos < LCD_PIXEL_HEIGHT ) )
	{
		*(pixel_t *)(CurrentFrameBuffer + LCD_BPP*(Xpos + (LCD_PIXEL_WIDTH*Ypos))) = color;
	}
}

uint16_t bsp_read_pixel(uint16_t Xpos, uint16_t Ypos)
{
	if ( ( Xpos < LCD_PIXEL_WIDTH ) && ( Ypos < LCD_PIXEL_HEIGHT ) )
	{
		return *(pixel_t *)(CurrentFrameBuffer + LCD_BPP*(Xpos + (LCD_PIXEL_WIDTH*Ypos)));
	}
	else
	{
		return 0;
...
void bsp_lcd_fill(uint16_t X1pos, uint16_t Y1pos,uint16_t X2pos, uint16_t Y2pos,uint16_t color)
{
	uint16_t x,y;
    
    if(X2pos>=LCD_PIXEL_WIDTH) X2pos=LCD_PIXEL_WIDTH-1;
    else if(Y2pos>=LCD_PIXEL_HEIGHT) Y2pos=LCD_PIXEL_HEIGHT-1;
    
    for(y=Y1pos;y<=Y2pos;y++)
    {
        for(x=X1pos;x<=X2pos;x++)
           bsp_draw_pixel(x,y,color); 
    }
}
