...
*               ÈìæÊé•|Â¢ûÂÄº÷µ|ÂºÄÊîæ|‰∏?Á´ã|ÂÆâÂÖ®|Ëá?Êúâ|Ëá?Áî±|ÁîüÊÄ?
*               www.gizwits.com
*
***********************************************************/
...
*
* @return 0Ôº? success; otherÔº? failure    
*/
...
*
* @return 0Ôº? suceess; otherÔº? failure
*/
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    {
        info->event[info->num] = EVENT_brightness;
        info->num++;
        dataPoints->valuebrightness = gizX2Y(brightness_RATIO,  brightness_ADDITION, issuedData->attrVals.valuebrightness); 
    }
    ...
    if(0x01 == issuedData->attrFlags.flagfan)
    {
        ...
        info->num++;
        dataPoints->valuefan = gizX2Y(fan_RATIO,  fan_ADDITION, issuedData->attrVals.valuefan); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    {
        GIZWITS_LOG("valuebrightness Changed\n");
        ret = 1;
    }
    ...
    if(last->valueBH1750 != cur->valueBH1750)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueBH1750 Changed\n");
            ret = 1;
        }
    }
    ...
    if(last->valuesoil != cur->valuesoil)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesoil Changed\n");
            ret = 1;
        }
    }
    if(last->valuedistant != cur->valuedistant)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuedistant Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuefan = gizY2X(fan_RATIO,  fan_ADDITION, dataPoints->valuefan); 
    devStatusPtr->valueBH1750 = gizY2X(BH1750_RATIO,  BH1750_ADDITION, dataPoints->valueBH1750); 
    devStatusPtr->valueCO2 = gizY2X(CO2_RATIO,  CO2_ADDITION, dataPoints->valueCO2); 
    ...
    devStatusPtr->valuesoil = gizY2X(soil_RATIO,  soil_ADDITION, dataPoints->valuesoil); 
    devStatusPtr->valuedistant = gizY2X(distant_RATIO,  distant_ADDITION, dataPoints->valuedistant); 
...
*           -1ÔºåInput Param Illegal
*           -2ÔºåSerial send faild
*/
...
*
* @return : 0,Return correct ;-1ÔºåReturn failure;-2ÔºåData check failure
*/
...
* Time-out 200ms no ACK resendÔºåresend two times at most
...
* @param[in] mode Ôº?0x0Ôº? reset the module ;0x01Ôº? SoftAp mode ;0x02Ôº? AirLink mode ;0x03Ôº? Production test mode; 0x04:allow users to bind devices
...
* @param [in] head : 
*
* @return 0, SuccessÔº? , other,Faild
*/
...
* @return 0Ôºåsuccess ;otherÔºåfailure
*/
